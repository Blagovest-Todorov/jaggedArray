...
namespace _05.JaggedArrays
{
    ...
    {
        static void Main(string[] args)  //jaged arrayy -array on which row the colums are different on different rows
        {
            // int[][] jagged = new int[3][]; // jagged array / we say how many rows, but we left the colums empty-no numbers size
            // jagged[0] = new int[3];   //on row with index 0 we make here 3 colums
